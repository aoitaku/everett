...
.command-palette(:class="compacted ? 'compacted' : ''")
  el-tabs(tab-position="left", style="height: 100%", @tab-click="tabSelected")
    el-tab-pane(ref="defaultTabPane")
      icon.message(slot="label", name="commenting-o")
      h1 メッセージ
      ul
        li
          | コマンド行（
    ...
    el-tab-pane
      ...
      h1 基本
      el-collapse
        el-collapse-item(v-for="item in basicCommands", :title="item.title", :name="item.name", :key="item.name")
          command-help(:item="item")
    el-tab-pane
      icon.screen(slot="label", name="desktop")
      h1 画面効果
      el-collapse
        el-collapse-item(v-for="item in screenCommands", :title="item.title", :name="item.name", :key="item.name")
          command-help(:item="item")
    el-tab-pane
      icon.image(slot="label", name="image")
      h1 画像
      el-collapse
        el-collapse-item(v-for="item in imageCommands", :title="item.title", :name="item.name", :key="item.name")
          command-help(:item="item")
    el-tab-pane
      icon.audio(slot="label", name="music")
      h1 オーディオ
      el-collapse
        el-collapse-item(v-for="item in audioCommands", :title="item.title", :name="item.name", :key="item.name")
          command-help(:item="item")
    el-tab-pane
      icon.video(slot="label", name="film")
      h1 ビデオ
      el-collapse
        el-collapse-item(v-for="item in videoCommands", :title="item.title", :name="item.name", :key="item.name")
          command-help(:item="item")
    el-tab-pane
      icon.system(slot="label", name="cog")
      h1 システム
      el-collapse
        el-collapse-item(v-for="item in systemCommands", :title="item.title", :name="item.name", :key="item.name")
          command-help(:item="item")
    el-tab-pane
